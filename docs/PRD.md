...
4. [The Core Model — Matter as Container](#4-the-core-model--matter-as-container)
...
An embedded AI layer reduces administrative overhead across document handling, deadline management, invoicing, and client communication. The AI is not legal advice — it is a practical multiplier that generates suggestions humans always confirm.
...
**Outputs:** organised matters, work completed on time, documents managed correctly, invoices sent and paid, clear client updates, a complete audit trail

The product has exactly three jobs:
1. **Organise work** — Matter system
...
|---------|------|---------------|
| Managing Partner | Firm owner / principal lawyer | Overview of all matters, financial health, team utilisation |
| Associate / Lawyer | Fee earner | Task lists, time logging, document access, deadline visibility |
| Legal Secretary / PA | Admin and support staff | Matter intake, scheduling, client communication, invoice dispatch |
| Client | External portal user | Status updates, document access, invoice payment via MoMo or card |

---

## 4. The Core Model — Matter as Container

Everything in the system revolves around one object: **the Matter**.

A Matter is a container that holds everything related to a single legal engagement:
...
- **Messages** — internal and client-facing
- **Audit log** — immutable record of who did what, when

If Matter is designed cleanly, the whole product stays coherent. Every other feature is either a view into a Matter, an action on a Matter, or a report across Matters.
...
### Domain 1 — Work (Matter Operations)

**Purpose:** Move matters forward without chaos.

#### Matter Management
- Create, edit, archive, and search matters with fields for matter type, practice area, status, court, suit number, opposing party, and opposing counsel
- Matter dashboard shows status, next deadline, unpaid balance, and recent activity at a glance
- Assign a lead lawyer and additional matter members with role-based visibility (lead / member / viewer)
- Bulk matter import via CSV for onboarding existing firms

#### Task Management
- Create tasks within a matter with title, description, assignee, due date, priority, and status
- Templates: create reusable task checklists per matter type (e.g. conveyancing checklist)
- Overdue task alerts via email and in-app notification
- Personal task view: all tasks assigned to the current user across all matters

#### Calendar & Deadlines
- Shared firm calendar with matter-linked events
...
#### Notes
- Rich-text notes on any matter, timestamped and attributed to the creator
- Pin important notes to the top of the matter
...
#### Document Management
- Upload PDFs, scans, and Office files to any matter
...
- Sub-second results; filter by matter, date range, document type, uploader
- Results scoped strictly to the user's own firm

#### Audit Trail
- Every create, update, delete, upload, and view action on any matter object is logged
...
- One-click time entry from any matter page; start/stop timer also available
...
#### Invoicing
- Generate an invoice from matter time and expense entries
...
- View matter timeline, status updates, and assigned lawyer
...
- Secure messaging thread with the firm
...
| **Matter status summary** | Matter activity log and notes | Plain-language summary of current status and next steps |
| **Invoice draft** | Logged time entries and expenses | Proposed line items and billing narrative for lawyer review |
| **Client update draft** | Recent matter activity | Draft client-facing update message for lawyer review |
| **Intake assist** *(Phase 2)* | Intake form responses | Pre-populated Matter, Client, and task checklist |
...
1. Lawyer uploads a PDF court order to a matter
...
6. Events are saved to the matter calendar and the firm-wide calendar
...
1. Lawyer logs time and expenses against the matter throughout the engagement
...
1. Lawyer posts a status update, shares a document, or dispatches an invoice from the matter view
2. Client portal timeline updates immediately
3. Client logs in, views status, downloads documents, or pays the invoice — without calling the firm
4. All client actions are saved to the matter audit log
...
**Work domain**
- Matter CRUD with all core fields
...
- Immutable audit trail on all matter objects
...
- Client portal: status, shared documents, invoices, messaging
- Email notifications
...
| Days from work complete to invoice sent | > 7 days typical | ≤ 2 days |
