...
interface CaseBillingClientProps {
  caseId: string;
  caseTitle: string;
...
export function CaseBillingClient({
  caseId,
  caseTitle,
  initialInvoices,
  initialTimeEntries,
}: CaseBillingClientProps) {
    ...
    setTimeEntries((current) => [
      {
        ...
        caseId,
        caseReference: current[0]?.caseReference ?? "CAS-NEW",
        caseTitle,
  ...
  return (
    <section className="case-billing-workspace">
      <div className="surface-card case-billing-panel">
        <div className="case-billing-header">
          <div className="case-billing-header-copy">
            <p className="eyebrow-label">Case Billing</p>
            <h2 className="section-title">Time &amp; Invoices</h2>
            <p className="case-tab-copy">
              Time entries, expenses, and invoice generation for {caseTitle}.
            </p>
            {unbilled.length > 0 ? (
              <div className="case-billing-header-meta">
                <span className="billing-unbilled-count">{unbilled.length} unbilled</span>
                <span className="case-inline-note">Unbilled total {formatGHS(unbilledTotal)}</span>
              </div>
            ) : null}
          </div>
        ...
        <div className="case-billing-section">
          <h3 className="case-billing-section-title">
            Time Entries
            {unbilled.length > 0 ? (
              <span className="billing-unbilled-count">{unbilled.length} unbilled</span>
            ) : null}
          ...
          {timeEntries.length === 0 ? (
            <div className="case-tab-empty billing-empty">
              No time entries yet. Log time to start tracking billable hours for this case.
            </div>
          ) : (
        ...
        {invoices.length > 0 ? (
          <div className="case-billing-section">
            <h3 className="case-billing-section-title">Invoices</h3>
            <div className="case-invoice-list">
              {invoices.map((inv) => (
                <CaseInvoiceRow key={inv.id} invoice={inv} />
              ))}
            </div>
          </div>
        ) : null}
...
function CaseInvoiceRow({ invoice }: { invoice: Invoice }) {
  return (
    <Link className="case-invoice-row" href={`/billing/${invoice.id}`}>
      <div>
      ...
      <div className="case-invoice-amounts">
        <span
      ...
      >
        <div className="task-modal-head">
          <div>
            ...
            <h3 className="case-title task-modal-title" id="time-entry-modal-title">
              New Time Entry
            </h3>
          </div>
          <button
