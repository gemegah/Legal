...
export function MatterNotesClient({
  initialNotes,
  matterId,
}: {
  ...
  function handleSelect(note: MatterNote) {
  ...
  function handleDuplicate(note: MatterNote) {
    const duplicate: MatterNote = {
      ...note,
      id: `note-${Math.random().toString(36).slice(2, 10)}`,
  ...
  function handleCopyLink(note: MatterNote) {
    if (typeof window === "undefined") {
      return;
    }
  ...
  return (
    <section className="matter-tab-panel notes-workspace">
      <div className="surface-card notes-hero">
        <div>
          <p className="eyebrow-label">Matter Notes</p>
        ...
        <div className="notes-hero-actions">
          <span className="matter-inline-note">Firm users only</span>
        ...
        <div className="surface-card notes-list-panel">
          {notes.length === 0 ? (
            <div className="empty-state notes-empty">
              No notes yet. Start with a private working note or pin the current matter strategy here.
            </div>
            ...
            <>
              ...
              <div className="notes-section">
                <div className="notes-section-header">
                  <h3 className="section-title">All Matter Notes</h3>
        ...
        <div className="surface-card notes-editor-panel">
          <div className="notes-editor-head">
            <div>
              ...
              <h3 className="section-title">{isCreating ? "Create private matter note" : selectedNote?.title ?? "Create note"}</h3>
            </div>
...
function NoteCard({
  ...
  note: MatterNote;
  onSelect: (note: MatterNote) => void;
  onTogglePin: (noteId: string) => void;
  onDuplicate: (note: MatterNote) => void;
  onCopyLink: (note: MatterNote) => void;
...
function noteToDraft(note: MatterNote): DraftState {
  return {
    title: note.title,
    body: note.body,
    tags: note.tags.join(", "),
